...
public class VR_Controller : MonoBehaviour
{
    ...
    private void SetWorldVRMode()
    {
        ...
        rec.transform.localScale = Constants.VR_CanvasScale;
        rec.sizeDelta = new Vector2(Constants.VR_CanvasWidth, Constants.VR_CanvasHeight);

        Transform rigTransform = transform.Find("[CameraRig]");
        canvas.transform.SetParent(rigTransform);
    }
